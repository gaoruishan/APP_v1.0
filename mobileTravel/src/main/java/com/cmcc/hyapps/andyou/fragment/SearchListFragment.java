 ...
 */
public class SearchListFragment extends BaseFragment implements /*DataLoader.DataLoaderCallback<QHScenic.QHScenicList>, */View.OnClickListener {
...
//        initData();
    ...
    private void loadNearScenics(String url) {
        ...
        if (empty_hint_view.isShown()){
            empty_hint_view.setVisibility(View.INVISIBLE);
        }
        ...
        try {
            for (int i = 0; i < response.length(); i++) {
                ...
                if ("entertainment".equals(type)) {
                    JSONObject jsonObj = response.getJSONObject(i);
                    QHEnjoy qhEnjoy = mGson.fromJson(jsonObj.toString(), QHEnjoy.class);
                    QHSearch qhSearch = new QHSearch(type,qhEnjoy);
                    mSearch.add(qhSearch);
                }
            }
