 ...
 */
public class GsonRequestForAESEncrpt<T> extends Request<T> {
     ...
     * @param listener
     */
    public GsonRequestForAESEncrpt(String url, Response.ErrorListener errorListener, Class<T> cls,
                       Response.Listener<T> listener,String key) {
        this(Method.GET, url, cls, null, listener, errorListener,key);
    }
     ...
     * @param listener
     */
    public GsonRequestForAESEncrpt(int method, String url, Class<T> cls, String requestBody,
...
//                            HttpHeaderParser.parseCacheHeaders(response));
            ...
            JSONObject jsonObj = new JSONObject(json);
////            AESResponse aesResponse = mGson.fromJson(json, AESResponse.class);
            String aes_string = jsonObj.getString("data");
            if (TextUtils.isEmpty(aes_string)){
                return null;
            }
            String aes_encrypt = AESEncrpt.Decrypt(aes_string, key);
            if (TextUtils.isEmpty(aes_encrypt)){
              return null;
            }
            T t = mGson.fromJson(aes_encrypt, mClass);
