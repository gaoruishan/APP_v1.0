...
import com.cmcc.hyapps.andyou.adapter.QHEntertainmentsDetailAdapter;
 ...
 * @author kuloud
 */
public class EntertainmentsDetailActivity extends BaseActivity implements View.OnClickListener, SharePopupWindows.OnSharePopupWindowsBack {
    private Context mContext;
    protected String mRequestTag = EntertainmentsDetailActivity.class.getName();
    private QHEnjoy routedetail;
    ...
    private QHEntertainmentsDetailAdapter qhRouteDetailAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_entertainments_detail);
        ButterKnife.inject(this);
        Intent intent = getIntent();
        tag = intent.getBooleanExtra("TAG", false);
//        Bundle route =  intent.getBundleExtra("entertainments");
//        routedetail = route.getParcelable("entertainments");
//        route_detail_id = route.getInt("id");
        route_detail_id = intent.getIntExtra("entertainment", 0);
...
//                if (share_media == SHARE_MEDIA.SINA || share_media == SHARE_MEDIA.WEIXIN || share_media == SHARE_MEDIA.WEIXIN_CIRCLE)
                    ToastUtils.show(EntertainmentsDetailActivity.this, "分享成功");
            }

            @Override
            public void shareFaild() {
//                ToastUtils.show(EntertainmentsDetailActivity.this, "分享失败");
            }
        });
        //  initData();
    }
    ...
    private void initData() {
        ...
        mLayoutManager = new LinearLayoutManager(EntertainmentsDetailActivity.this);
        mRecyclerView.setLayoutManager(mLayoutManager);
        //如果可以确定每个item的高度是固定的，设置这个选项可以提高性能
        //mRecyclerView.setHasFixedSize(true);
        //创建并设置Adapter  http://112.54.207.48/api/entertainments/
//        String url = ServerAPI.Route.buildItemDetailUrl(route_detail_id + "");
        String url = ServerAPI.Entertainments.buildItemDetailUrl(route_detail_id + "");

        RequestManager.getInstance().sendGsonRequest(Request.Method.GET, url,
                QHEnjoy.class, null,
                new Response.Listener<QHEnjoy>() {
                    @Override
                    public void onResponse(QHEnjoy response) {

                        routedetail = response;
                        qhRouteDetailAdapter = new QHEntertainmentsDetailAdapter(EntertainmentsDetailActivity.this, routedetail.entertainment_info, response);
                        if (routedetail.entertainment_info != null && routedetail.entertainment_info.size() != 0) {
                        ...
                        tv_comment.setOnClickListener(EntertainmentsDetailActivity.this);
                        tv_praise.setOnClickListener(EntertainmentsDetailActivity.this);
...
//        url = ServerAPI.BASE_URL + "routes/"+routedetail.id+"/comments/?limit="+limit+"/";
        url = ServerAPI.BASE_URL + "entertainments/" + routedetail.id + "/comments/?limit=" + limit + "/";
...
//        tv_comment.setText(""+response.results.size());
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_comment:
                Intent intent = new Intent(EntertainmentsDetailActivity.this, CommentListActivity.class);
                intent.putExtra("url", url);
                intent.putExtra("object_id", routedetail.id);
                intent.putExtra("ctype", Integer.parseInt(ServerAPI.Comments.Type.ENTERTAINMENTS.value()));
                startActivityForResult(intent, REQUEST_CODE_COMMENT_LIST);
                break;
            case R.id.tv_praise:
                AnimUtils.doScaleFadeAnim(v);
                if (routedetail.voted == 1) {//已点赞
                    tv_praise.getCompoundDrawables()[1].setLevel(2);
                    tv_praise.setEnabled(false);
                    ToastUtils.show(getApplicationContext(), "您已经点赞过，不能再次点赞！");
                } else {
                    if (!UserManager.makeSureLogin(EntertainmentsDetailActivity.this, REQUEST_CODE_LOGIN_NEW_COMMENT)) {
                        voteComment();
                    }
                }
                break;
    ...
    private void postCollection() {
        ...
        CommentObj obj = new CommentObj("" + routedetail.id, ServerAPI.Comments.Type.ENTERTAINMENTS.value());
        Gson gson = new Gson();
...
//                        }
                            ...
                            AppUtils.handleResponseError(EntertainmentsDetailActivity.this, error);
                        }
...
//                        }
    ...
    private void gotoShare(QHEnjoy routedetail, String type) {
        if (routedetail != null && routedetail.getEntertainment_info() != null && routedetail.getEntertainment_info().size() > 1) {
            QHEnjoyInfo item = routedetail.getEntertainment_info().get(1);
            if (item != null) {
