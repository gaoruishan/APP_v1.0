...
public class FoodFragment extends BaseFragment implements OnClickListener,ChoosePopupWindow.OnPopupWindowsClickListener {
    ...
    private FoodListFragment mSearchFoodListFragment;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_market_food, container, false);
    ...
    private void UploadRecyclerView() {
        ...
        mSearchFoodListFragment = (FoodListFragment) getFragmentManager().findFragmentByTag(TAG);
        String url = ServerAPI.RestaurantList.buildSearchCommentUrl(condition);
        mSearchFoodListFragment.loadList(url);
        mSearchFoodListFragment.reload(DataLoader.MODE_REFRESH);
    }

    private void chooseConditionToUploadRecyclerView(int distancePosition,int pricePosition){
        mSearchFoodListFragment = (FoodListFragment) getFragmentManager().findFragmentByTag(TAG);
        mSearchFoodListFragment.loadList(setUrl(distancePosition,pricePosition));
        mSearchFoodListFragment.reload(DataLoader.MODE_REFRESH);
    }
    private String setUrl(int distancePosition,int pricePosition){
        ...
        if (distancePosition == distances.length-1 && pricePosition == prices.length -1)
            url = ServerAPI.RestaurantList.buildMutilConditionMarketAllNotLimitUrl(mLatitude,mLongitude,"average");
        if (distancePosition == distances.length-1 && pricePosition != prices.length -1)
            url = ServerAPI.RestaurantList.buildMutilConditionMarketNoLimtDistanceUrl(mLatitude, mLongitude, "average", prices[pricePosition]);
        if (distancePosition != distances.length-1 && pricePosition == prices.length -1)
            url = ServerAPI.RestaurantList.buildMutilConditionMarketNoLimitpriceUrl(mLatitude, mLongitude, distances[distancePosition], "average");
        if (distancePosition != distances.length-1 && pricePosition != prices.length -1)
            url = ServerAPI.RestaurantList.buildMutilConditionMarketUrl(mLatitude, mLongitude, distances[distancePosition], "average", prices[pricePosition]);
        return  url;
    }
    @Override
...
//        mFoodListFragment.attachLoadingProgress(mLoadingProgress);
        ...
        float mLatitude = (float)LocationUtil.getInstance(getActivity()).getLatitude();
        float mLongitude = (float)LocationUtil.getInstance(getActivity()).getLongitude();
        String url = ServerAPI.RestaurantList.buildMutilConditionMarketNoLimitpriceUrl(mLatitude, mLongitude, "1000", "average");
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.fragment_search_et:
                ...
                searchIntent.putExtra(Const.EXTRA_STYPE, Const.MARKET_FOOD);
