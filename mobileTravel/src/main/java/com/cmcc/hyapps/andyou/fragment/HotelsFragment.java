...
public class HotelsFragment extends BaseFragment implements OnClickListener,ChoosePopupWindow.OnPopupWindowsClickListener {
    private final String TAG = "HotelsFragment";
    ...
    private HotelsListFragment mSearchHotelsListFragment;
    private HotelsListFragment mHotelsListFragment;
    private ChoosePopupWindow chooseDistancePopupWindow,choosePricePopupWindow;
    private String[] prices = new String[]{ "0_50", "50_100", "100_200","200_300", "300_inf","不限"};
    private String[] distances = new String[]{"500", "1000", "2000","5000", "不限"};

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_market_hotels_layout, container, false);
    ...
    private void UploadRecyclerView() {
        ...
        mSearchHotelsListFragment = (HotelsListFragment) getFragmentManager().findFragmentByTag(TAG);
        String url = ServerAPI.HotelsList.buildSearchCommentUrl(condition);
        mSearchHotelsListFragment.loadList(url);
        mSearchHotelsListFragment.reload(DataLoader.MODE_REFRESH);
    }

    private void chooseConditionToUploadRecyclerView(int distancePosition,int pricePosition){
        mSearchHotelsListFragment = (HotelsListFragment) getFragmentManager().findFragmentByTag(TAG);
        mSearchHotelsListFragment.loadList(setUrl(distancePosition, pricePosition));
        mSearchHotelsListFragment.reload(DataLoader.MODE_REFRESH);
    }
    private String setUrl(int distancePosition,int pricePosition){
        ...
        if (distancePosition == distances.length-1 && pricePosition == prices.length -1)
         url = ServerAPI.HotelsList.buildMutilConditionMarketAllNotLimitUrl(mLatitude,mLongitude,"average");
        if (distancePosition == distances.length-1 && pricePosition != prices.length -1)
            url = ServerAPI.HotelsList.buildMutilConditionMarketNoLimtDistanceUrl(mLatitude, mLongitude, "average", prices[pricePosition]);
        if (distancePosition != distances.length-1 && pricePosition == prices.length -1)
            url = ServerAPI.HotelsList.buildMutilConditionMarketNoLimitpriceUrl(mLatitude, mLongitude, distances[distancePosition], "average");
        if (distancePosition != distances.length-1 && pricePosition != prices.length -1)
            url = ServerAPI.HotelsList.buildMutilConditionMarketUrl(mLatitude, mLongitude, distances[distancePosition], "average", prices[pricePosition]);
        return  url;
    }
    @Override
    ...
    private void showFoodList() {
        mHotelsListFragment = new HotelsListFragment();
//        mFoodListFragment.attachLoadingProgress(mLoadingProgress);
        getFragmentManager().beginTransaction().add(R.id.live_container, mHotelsListFragment, TAG).commitAllowingStateLoss();
        float mLatitude = (float)LocationUtil.getInstance(getActivity()).getLatitude();
        float mLongitude = (float)LocationUtil.getInstance(getActivity()).getLongitude();
          String url = ServerAPI.HotelsList.buildMutilConditionMarketNoLimitpriceUrl(mLatitude, mLongitude, "1000", "average");
        mHotelsListFragment.initLoacation(city, city_en,url);
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.fragment_search_et:
                ...
                searchIntent.putExtra(Const.EXTRA_STYPE,Const.MARKET_HOTEL);
