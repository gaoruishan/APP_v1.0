...
public class GuiderMarketCommentListFragment extends BaseFragment implements OnClickListener,DataLoaderCallback<Comment.CommentList> {
    ...
    private QHMarketShop marketShop;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        marketShop = getArguments().getParcelable(Const.QH_SECNIC);
        mId = getArguments().getInt(Const.QH_SECNIC_ID);
        initViews();
//        reload();
//        loadCommentData("北京",DataLoader.MODE_REFRESH);
...
//        mActionBar.getRightView().setImageResource(R.drawable.);
    ...
    private void loadCommentData(String cityName,int mode) {
        Log.e("-------","loadNearScenics--");
        if(null==marketShop) marketShop = new QHMarketShop();
        marketShop.id = mId;
        String  url  = ServerAPI.User.buildMarketCommentUrl(mId);
    //    mLoadingProgress.setVisibility(View.VISIBLE);
//      commentListView.setVisibility(View.GONE);
...
//            return;
           // mEmptyHintView.setVisibility(View.VISIBLE);
            ...
            else mEmptyHintView.setVisibility(View.GONE);
        }
        //date == null  is load more
//        if(data == null)
//            mEmptyHintView.setVisibility(View.GONE);
//        else if (data.results == null || data.results.isEmpty()){
//            mEmptyHintView.setVisibility(View.VISIBLE);
//        }

        if (data.results == null || data.results.isEmpty()) {
            if (mode == DataLoader.MODE_LOAD_MORE){
                ToastUtils.AvoidRepeatToastShow(GuiderMarketCommentListFragment.this.getActivity(), R.string.msg_no_more_data, Toast.LENGTH_SHORT);
            }else {
                mEmptyHintView.setVisibility(View.VISIBLE);
            }
            return;
                       ...
                       // mVoteText.setEnabled(false);
                        mVoteText.setOnClickListener(new OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ToastUtils.AvoidRepeatToastShow(GuiderMarketCommentListFragment.this.getActivity(),R.string.error_already_voted,Toast.LENGTH_SHORT);
                            }
                        });
                    }
    ...
    private void postNewComment() {
        ...
        commentEdit.putExtra(Const.EXTRA_ID, marketShop.id);
        commentEdit.putExtra(Const.MARKET_HOTEL,Const.MARKET);
